...
#cmt_local_tagfile_install_more_includes = $(MCParamsSvc_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(MCParamsSvc_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)MCParamsSvc_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)MCParamsSvcsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "MCParamsSvc" = ""; then \
	    if test -d ../MCParamsSvc; then \
	      echo "Installing files from more ../MCParamsSvc to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/MCParamsSvc; then mkdir -p $(install_include_dir)/MCParamsSvc; fi;\
	        $(install_command) ../MCParamsSvc $(install_include_dir);\
	    else \
	      echo "No more MCParamsSvc include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/MCParamsSvc"; \
	  $(uninstall_command) "$(install_include_dir)/MCParamsSvc/*" ; \
	  if test -f $(install_include_dir)/MCParamsSvc.cmtref ; then \
	    echo "Removing $(install_include_dir)/MCParamsSvc.cmtref"; \
	    eval rm -f $(install_include_dir)/MCParamsSvc.cmtref ; \
	  fi; \
	fi
